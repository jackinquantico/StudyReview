package com.kh.part_14.afterJDBC.model.vo;
 ...
 * 
 * 유사용어 : DTO (Data Transfer Object)
 * 
 * VO 조건
 * 1) 반드시 캡슐화 적용 (필드는 private, getter / setter 메소드)
 * 2) 기본생성자 및 매개변수 생성자를 작성할 것
 * +  toString() 오버라이딩 => Object 클래스 (모든 클래스의 최상위 부모)에서 제공하는 메소드
 * 						 => 원래 역할 : 주소값 return / 재정의할 역할 : 모든 필드 값을 하나의 문자열로 연이어서 return
 */
...
public class Member {
	...
	// => 오라클에서는 문자 / 문자열 구분이 없기 때문에 자바에서도 String으로 정의
